...
export const CartProvider = ({ children }) => {
  ...
  useEffect(() => {
    const loadCart = async () => {
      if (user && token) {
        try {
          const res = await fetch(`${API_BASE_URL}/api/cart/manage.php`, {
    ...
    const syncCart = async () => {
      if (user && token && cartItems.length > 0) {
        ...
        if (guestCart) {
          ...
          try {
            await fetch(`${API_BASE_URL}/api/cart/manage.php`, {
              method: 'POST',
              headers: {
  ...
  const addToCart = async (product, qty = 1) => {
    ...
    if (user && token) {
      try {
        ...
        const data = await res.json();
        if (data.success) {
          const getCart = await fetch(`${API_BASE_URL}/api/cart/manage.php`, {
            headers: { 'Authorization': `Bearer ${token}` }
          });
  ...
  const removeFromCart = async (productId) => {
    if (user && token) {
      try {
        const res = await fetch(`${API_BASE_URL}/api/cart/manage.php?product_id=${productId}`, {
          method: 'DELETE',
          headers: { 'Authorization': `Bearer ${token}` }
        });
        const data = await res.json();
  ...
  return (
    <CartContext.Provider value={{ cartItems, addToCart, updateQuantity, removeFromCart, clearCart, getCartCount, getCartTotal, isCartOpen, setIsCartOpen }}>
      {children}
