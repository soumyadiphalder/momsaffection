...
try {
    ...
    $paymentMethod = trim($data['payment_method'] ?? 'Razorpay');
    if (!in_array($paymentMethod, ['Razorpay', 'COD'])) {
    ...
    foreach ($cartItems as $item) {
        $pStmt = $pdo->prepare("SELECT PRODUCT_SELL_PRICE, PRODUCT_STOCK, PRODUCT_NAME FROM MM_PRODUCT WHERE PRODUCT_ID = ? FOR UPDATE");
        ...
        $totalAmount += $product['PRODUCT_SELL_PRICE'] * $item['QUANTITY'];
    }
    ...
    $remarks = ($paymentMethod === 'COD') ? 'Order placed. Cash on Delivery selected.' : 'Order placed. Awaiting payment details.';
    $stmtStatus = $pdo->prepare("INSERT INTO MM_ORDER_STATUS (STATUS_ID, ORDER_ID, STATUS_NAME, STATUS_DATE, REMARKS) VALUES (?, ?, 'PENDING', NOW(), ?)");
    $stmtStatus->execute([$statusId, $orderId, $remarks]);
    
    // 4. Create pending payment records
    $paymentId = 'pay_' . bin2hex(random_bytes(8));
    $stmtPayment = $pdo->prepare("INSERT INTO MM_PAYMENT (PAYMENT_ID, ORDER_ID, PAYMENT_METHOD, PAYMENT_AMOUNT, PAYMENT_STATUS, PAYMENT_DATE) VALUES (?, ?, ?, ?, 'PENDING', NOW())");
    $stmtPayment->execute([$paymentId, $orderId, $paymentMethod, $totalAmount]);
    
    // 5. Clear cart only for COD orders
